...
## Dessiner des formes libres (modes viewport)

Les outils ci-dessus génèrent des volumes à partir de champs de saisie. Les deux modes
suivants, eux, se pilotent à la souris **dans le viewport**. On y entre par un **bouton du
panneau** — sections « Dessin libre » et « Découpe » — et on en sort par le bouton
« Quitter » juste en dessous, ou avec Échap.

(Les deux modes apparaissent aussi dans la barre de modes du viewport, mais ils y portent
la même icône et sont difficiles à distinguer. Passe par les boutons du panneau.)

### Dessin Blockout

Pour tracer un sol, une plateforme ou un mur de n'importe quelle forme, au lieu de le
composer avec des boîtes.

1. Dans le panneau, section **Dessin libre** → bouton **Activer le mode dessin**.
2. **Clic gauche** pose un point. Les points se posent sur le plan de dessin, dont la
   hauteur est rappelée en bas de l'écran.
3. **Clic sur le premier point**, ou **Entrée**, ferme le contour.
4. **Ctrl maintenu** (à partir du 3e point) force les angles droits : chaque mur est
   perpendiculaire ou parallèle au **premier segment** que tu as tracé. Le bâtiment peut
   donc être orienté en biais, tous ses angles restent à 90°. Le HUD indique quand la
   contrainte est active. Le segment de fermeture, lui, relie le dernier point au premier
   tel quel — si tu veux qu'il soit droit aussi, place tes points en conséquence.
5. **Ctrl+F** (à partir de 4 points) déplace le dernier point pour que la fermeture
   retombe d'équerre, et ferme le contour dans la foulée. L'endroit où tu avais posé ce
   dernier point n'a pas d'importance : il est remplacé par celui qui ferme le rectangle.
   Attention à ce que ça ne fait pas — Ctrl+F ne déplace qu'un point, donc il redresse
   l'angle de ce point et celui du premier ; si les murs d'avant n'étaient pas d'équerre,
   ceux-là ne le deviennent pas. Trace avec Ctrl et tout est droit.
6. Bouge la souris pour régler l'épaisseur. Vers le haut → le volume monte ; vers le bas
   → il descend. **Ctrl** aimante la valeur sur la grille.
7. **Clic gauche** ou **Entrée** valide. L'acteur est créé.

**Retour arrière** retire le dernier point. **Échap** efface le tracé en cours, puis, si
le tracé est déjà vide, quitte le mode.

Le contour doit être **simple** : deux arêtes ne doivent pas se croiser. Un tracé en
« 8 » est refusé, et la ligne de statut le dit.

### Découpe Blockout

Pour percer une porte, une fenêtre ou une trappe dans une surface déjà posée — sans
découper à la main ni empiler des boîtes autour du trou.

1. Dans le panneau, section **Découpe** → bouton **Activer le mode decoupe**.
2. Survole la face à percer : son contour s'allume, et les trous déjà percés
   apparaissent.
3. **Clic gauche** pose les points de l'ouverture, sur cette face.
4. **Clic sur le premier point**, ou **Entrée**, perce.

**Retour arrière** retire le dernier point. **Échap** efface, puis quitte.

Ce qui est refusé, et pourquoi — le message apparaît en jaune en haut de l'écran :

| Message | Ce qu'il faut faire |
|---|---|
| Contour croisé | Le tracé se recoupe. Recommence sans croiser les arêtes. |
| Ouverture hors de la surface | Un point dépasse du bord. Reste à l'intérieur. |
| Ouverture superposée | Elle chevauche un trou existant. Perce ailleurs, ou refais un seul trou plus grand. |
| Surface non compatible | La Découpe ne travaille que sur des **boîtes** et sur les panneaux qu'elle a elle-même produits. Un cylindre ou un mesh d'art n'est pas découpable. |

La découpe est **annulable** (Ctrl+Z) comme n'importe quelle édition, et **rééditable** :
on peut percer plusieurs ouvertures dans la même surface, l'une après l'autre.

Le trou est un vrai trou dans le maillage **et** dans la collision : un personnage
traverse réellement une porte percée.
...
Une modification du panneau lui-même (nouvel outil, correction d'un bug) demande une recompilation
côté développeur : signale-le à Thomas, tu recevras une nouvelle version du dossier à remplacer.

Pour l'installation du plugin, voir `LISEZ-MOI.txt` dans le même dossier.
